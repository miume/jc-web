    ...
    render(){
        ...
        return(
           <Form horizontal='true' onSubmit={() => this.handleSubmit()}>
               <FormItem label='样本点名称' labelCol={{span:5}} wrapperCol={{span:14}} required>{/*required说明此项为必填项*/}
                 {getFieldDecorator('factoryName',{
                    initialValue: '',
                    rules:[{required:true,message:'样本点名称不能为空'}]
                 })(
                     <Input placeholder='请输入样本点名称'></Input>
                 )}
               </FormItem>
           </Form>
        );
