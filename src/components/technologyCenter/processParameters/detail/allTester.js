...
class AllTester extends React.Component{
    ...
    render(){
        const examineData = this.props.examineData ? this.props.examineData : data;
        const count = examineData?examineData.length:0;
        return (
            <div className={examineData?'check-detail':'hide'}>
                <div className={'process-parameters-detail-flex'} id={`check-detail-div`+this.props.dataId}>
                    {
                        examineData?
                            examineData.map((e,index)=>(
                                <div className='process-parameters-detail-flex'>
                                    <div className={'process-parameters-detail-flex'} key={index}>
                                        <div>
