                            ...
                            this.dataBottom?this.dataBottom.map((item,index)=>{
                                return(
                                    <span className='positive-process-add-onLine-font' key={index}>
                                        {item.materialName} : <Input placeholder='请输入' name={`${length+index}-${item.materialName}`} onChange={this.inputChange} key={item.code} defaultValue={item.value} style={{width:'150px',marginRight:'20px'}} suffix="kg"/>
                                    </span>
                                )
                            }):null
                          }
