...
class AddSearch extends Component{
    ...
    render(){
        ...
        let {timeDisabled} = this.props,subLength=this.props.subLength===undefined?undefined:this.props.subLength;
        return(
            <div className={this.props.flag?'':'hide'}>
                ...
                <span>结束时间 : </span>&nbsp;<DatePicker disabledDate={this.props.disabledDate} value={endDate} onChange={this.props.endChange} className='process-add-head-date' style={{marginRight:'20px'}} placeholder='请选择结束日期' disabled={!timeDisabled}/>
                
                {!this.props.disabledDateFlag?<Button type='primary' className='button' onClick={this.props.search} disabled={!timeDisabled}>确定</Button>
                :<Popconfirm title={`与上期结束时间间隔${subLength}天，确定继续吗?`} onConfirm={this.props.search} okText="确定" cancelText="再想想" >
                    <Button type='primary' className='button' style={this.props.editFlag?{display:'none'}:{}}  disabled={!timeDisabled}>确定</Button>
                </Popconfirm>}
                &nbsp;
            </div>
        );
